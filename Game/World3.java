import greenfoot.*;  // (World, Actor, GreenfootImage, Greenfoot and MouseInfo)

/**
 * Write a description of class world3 here.
 * 
 * @author (your name) 
 * @version (a version number or a date)
 */
public class World3 extends WorldA
{

    /**
     * Constructor for objects of class world3.
     * 
     */
    public World3(){
        this(0, 0, 0, 3);
    }

    public World3(int gold, int power1, int power2, int screen)
    {
        super(gold, power1, power2, screen);

        addObject(ground2, ground2.getImage().getWidth()/2, 696);
        addObject(ground3, getWidth()/2, 503);
        addObject(ground4, getWidth()-ground4.getImage().getWidth()/2, 314);

        addObject(mario, 35, ground2.getY() - ground.getImage().getHeight()/2 - mario.getImage().getHeight()/2 + 1);
        addObject(pPlant, ground3.getX(), ground3.getY()-pPlant.getImage().getHeight()/2-ground3.getImage().getHeight()/2);
        addObject(pipe, pPlant.getX(),ground3.getY()-pipe.getImage().getHeight()/2-ground3.getImage().getHeight()/2+28);
        addObject(goomba, 398, ground3.getY()-ground3.getImage().getHeight()/2- goomba.getImage().getHeight()/2);
        addObject(goomba2, 816, 2+ ground4.getY()-ground4.getImage().getHeight()/2 - goomba2.getImage().getHeight()/2);

        addObject(new Block(12), 240, 502);
        addObject(new Block(11), 512, 202);
    }
}
